...
namespace CreaJuego.Web
{
    ...
    {
        RuntimeAuthoringController c;Font font;Transform catalog,list,properties,editTop;Text status,save;Button modeButton;Toggle snap;Action<GameItem> selectionChanged;bool worldSelected;string appearanceSearch="";ItemKind? searchKind;
        ...
        void OnSelection(GameItem item){if(item!=null)worldSelected=false;if(item!=null&&searchKind!=item.definition.kind){appearanceSearch="";searchKind=item.definition.kind;}Refresh();}
        public void Build()
        {
        ...
        public void Refresh()
        {
            if(c==null||catalog==null)return;float listScroll=list.parent.GetComponent<ScrollRect>().verticalNormalizedPosition;Clear(catalog);Clear(list);Clear(properties);
            ...
            int row=0;foreach(var data in c.Project.objects){var captured=data;var def=c.Find(data.definitionId);ButtonAt(list,def!=null?def.displayName:data.definitionId,new Vector2(0,row++*-31),()=>Select(captured.instanceId),new Vector2(195,29));}ResizeContent(list,row*31);list.parent.GetComponent<ScrollRect>().verticalNormalizedPosition=listScroll;
            int py=0;if(worldSelected)WorldProperties(ref py);else{var item=c.Selection.SelectedItem;var data=c.SelectedData();if(item==null||data==null){Label(properties,"Elige un elemento o abre Nivel.",15,new Vector2(4,-py),new Vector2(245,45));py+=50;}else{Label(properties,item.definition.displayName.ToUpperInvariant(),19,new Vector2(4,-py),new Vector2(245,34));py+=38;ItemProperties(item.definition.kind,data,ref py);Appearance(item.definition.kind,data,ref py);}}
        ...
        Text Label(Transform parent,string value,int size,Vector2 pos,Vector2 dimensions){var go=New("Texto",parent);var text=go.AddComponent<Text>();text.font=font;text.fontSize=size;text.text=value;text.color=Color.white;text.alignment=TextAnchor.MiddleLeft;Rect(go.transform).anchoredPosition=pos;Rect(go.transform).sizeDelta=dimensions;return text;}
        static GameObject New(string name,Transform parent){var go=new GameObject(name,typeof(RectTransform));go.transform.SetParent(parent,false);var rect=(RectTransform)go.transform;rect.anchorMin=rect.anchorMax=rect.pivot=Vector2.zero;return go;}
        static RectTransform Rect(Transform transform)=>(RectTransform)transform;static void Anchor(RectTransform rect){rect.anchorMin=Vector2.zero;rect.anchorMax=Vector2.one;rect.offsetMin=rect.offsetMax=Vector2.zero;}static void Clear(Transform transform){for(int i=transform.childCount-1;i>=0;i--)Destroy(transform.GetChild(i).gameObject);}
    }
