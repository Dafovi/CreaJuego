// Platformer Game Kit // https://kybernetik.com.au/platformer // Copyright 2021-2025 Kybernetik //
...
namespace PlatformerGameKit
{
    ...
    partial class HitData
    {
        ...
        public class Drawer
        {
            ...
            public static void OnPreviewSceneGUI(
                ref HitData[] hits,
                TransitionPreviewDetails details,
            ...
            private static void DoSceneOverlayGUI(
                ref HitData[] hits,
                TransitionPreviewDetails details,
                float time,
                bool isAnyActive)
            {
                ...
                using (var content = PooledGUIContent.Acquire("Add Hit"))
                {
            ...
            private static void AddHit(
                ...
                float time)
            {
                TransitionDrawer.Context.Property.RecordUndo("Add Hit");
                ...
                if (AnimancerUtilities.TryGetFrameRate(TransitionDrawer.Context.Transition, out var frameRate))
                {
                ...
                if (hits.Length > 0)
                {
                    ...
                    hit.Force = copyFrom.Force;
                    hit.Angle = copyFrom.Angle;
                }
                else
                {
            ...
            private static void HandleAddRemovePoints(HitData[] hits, TransitionPreviewDetails details, float time)
            {
                ...
                Handles.color = new(0.75f, 1, 0.75f);
                ...
                if (isClosetoPoint)
                {
                    ...
                    {
                        TransitionDrawer.Context.Property.RecordUndo("Remove Hit Point");
                ...
                else
                {
                    if (currentEvent.type == EventType.MouseDown)
                    {
                        ...
                        TransitionDrawer.Context.Property.RecordUndo("Add Hit Point");
            ...
            private static void DoPreviewSceneGUI(HitData hit, TransitionPreviewDetails details)
            {
                ...
                for (int i = 0; i < count; i++)
                {
                    ...
                    if (EditorGUI.EndChangeCheck())
                    {
                        ...
                        TransitionDrawer.Context.Property.RecordUndo("Edit Hit Area");
                        area[i] = transform.InverseTransformPoint(point);
                        TransitionDrawer.Context.Property.serializedObject.Update();
                    }

                    var direction = AngleToDirection(hit.Angle, transform.rotation, false);
                    Handles.DrawLine(point, point + (Vector3)direction);
                }

                Handles.color = new(0.25f, 1, 0.25f);
            ...
            private static void GetPointsInWorldSpace(
                Vector2[] local,
                Vector3[] world,
                Transform transform,
                bool wrapLast)
            ...
            public static bool TryEvaluateProximityToPolyLine(
                Vector3[] vertices,
                int vertexCount,
                float maxDistance,
                out int index,
