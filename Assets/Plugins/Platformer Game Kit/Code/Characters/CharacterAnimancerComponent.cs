...
{
    /// <summary>
    /// An <see cref="AnimancerComponent"/>
    /// which also manages a <see cref="SpriteRenderer"/> and hit boxes.
    /// </summary>
    /// <remarks>
    /// <strong>Documentation:</strong>
    /// <see href="https://kybernetik.com.au/platformer/docs/characters/animancer">
    /// Animancer Component</see>
    /// </remarks>
    ...
    //[PlatformerHelpUrl(typeof(CharacterAnimancerComponent))]// Unity 2022 warns about multiple Help URLs due to inheritance.
    public class CharacterAnimancerComponent : AnimancerComponent
    {
        ...
        /// <see cref="AnimancerEvent.Current"/> state.
        /// </summary>
        public static CharacterAnimancerComponent GetCurrent()
            => Get(AnimancerEvent.Current.State);

        /// <summary>Returns the <see cref="CharacterAnimancerComponent"/> associated with the `node`.</summary>
        public static CharacterAnimancerComponent Get(AnimancerNode node)
            => Get(node.Graph);

        /// <summary>Returns the <see cref="CharacterAnimancerComponent"/> associated with the `animancer`.</summary>
        public static CharacterAnimancerComponent Get(AnimancerGraph animancer)
        ...
        public void AddHitBox(HitData data)
        {
            if (_IgnoreHits == null)
            {
                ObjectPool<Dictionary<HitData, HitTrigger>>.DefaultInstance.Acquire(out _ActiveHits);
                SetPool<Hit.ITarget>.Instance.Acquire(out _IgnoreHits);
            }
        ...
        public void EndHitSequence()
        {
            ...
            ClearHitBoxes();
            ObjectPool<Dictionary<HitData, HitTrigger>>.DefaultInstance.Release(ref _ActiveHits);
            SetPool<Hit.ITarget>.Instance.Release(ref _IgnoreHits);
        }
