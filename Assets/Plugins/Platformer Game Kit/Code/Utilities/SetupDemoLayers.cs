...
namespace PlatformerGameKit
{
    ...
    /// <see href="https://kybernetik.com.au/platformer/docs/other/layers">
    /// Physics Layers</see>
    /// </remarks>
    ...
    [PlatformerHelpUrl(typeof(SetupDemoLayers))]
    [DefaultExecutionOrder(DefaultExecutionOrder)]
    public class SetupDemoLayers : MonoBehaviour
    {
        /************************************************************************************************************************/
        #region Fake Layers
        /************************************************************************************************************************/
        ...
        private static int DemoCharacterLayer
            => LayerMask.NameToLayer(DemoCharacterLayerName);
        private static int DemoHitLayer
            => LayerMask.NameToLayer(DemoHitLayerName);
        private static int DemoHitTargetLayer
            => LayerMask.NameToLayer(DemoHitTargetLayerName);
        ...
        private sealed class Editor : UnityEditor.Editor
        {
            ...
            private static int ProperCharacterLayer
                => LayerMask.NameToLayer(ProperCharacterLayerName);
            private static int ProperHitLayer
                => LayerMask.NameToLayer(ProperHitLayerName);
            private static int ProperHitTargetLayer
                => LayerMask.NameToLayer(ProperHitTargetLayerName);
        ...
        {
            var readMe = Animancer.Editor.AnimancerEditorUtilities.FindAssetOfType<PlatformerGameKitReadMe>();
            if (readMe == null)
            {
                Debug.LogWarning($"Can't automatically swap existing objects to the newly created layers" +
                    $" because the {nameof(PlatformerGameKitReadMe)} asset wasn't found" +
                    $" (it's used to determine the base folder to modify assets in).");
                return;
            }
